const projectDetails = (project, edit = false) => {
  const deleteBtn = `<button id="project-delete" type="button" data-project='${project.id}' class='action-btn delete-btn'>Delete</button>`;
  const editBtn = `<button id="project-update" type="button" data-project='${project.id}' class='action-btn'>Update Project</button>`;
